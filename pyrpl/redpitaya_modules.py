...
class BaseModule(object):
    ...
    #fundamental functionality to read and write unsigned integer (32 bits wide) arrays
...
class Scope(BaseModule):
    ...
    def rawdata_ch1(self):
        #return np.array([self.to_pyint(v) for v in self.reads(0x10000, self.data_length)],dtype=np.int32)
    ...
    def rawdata_ch2(self):
        #return np.array([self.to_pyint(v) for v in self.reads(0x20000, self.data_length)],dtype=np.int32)
    ...
    def arm(self,frequency=None,trigger_source=1,trigger_delay = None):
        ...
        if trigger_delay is None:
            ...
            self.trigger_delay = np.int(np.round(trigger_delay))#*self.data_length))
        self.trigger_source = trigger_source
        self.arm_trigger()
...
class ASG(BaseModule):
    ...
    def scopetrigger_phase(self): 
        ...
        return np.float(self.to_pyint(self.read(0x114+self.value_offset),bitlength=14))/2**14*360.0
    ...
    def enable_advanced_trigger(self, frequency, amplitude, duration, invert=False, autorearm=False):
        self.setup_cosine(frequency=frequency, amplitude=amplitude, onetimetrigger=False, offset=0, data=True)
    ...
    @property
    def data(self):
        x = np.array(self.reads(self.data_offset, self.data_length),dtype=np.int32)
        x[x>=2**13] -= 2**14
        return x
    ...
    @property
    def frequency(self):
            return float(self.counter_step)/float(self.counter_wrap+2**16)/8e-9*self._frequency_correction
    @frequency.setter
    def frequency(self, v):
        v=float(v)/self._frequency_correction
        self.counter_step = np.long(np.round(float(v)*8e-9*(float(self.counter_wrap+2**16))))
        
    def setup_cosine(self,frequency=1,amplitude=1.0,onetimetrigger=False,offset=0,data=True):
        ...
        if data:
            self.data = np.array(np.round(-(2**13-1)*np.cos(np.linspace(0,2.0*np.pi,2**14,endpoint=False))),dtype=np.int32)
        self.start_offset = 0
    ...
    def setup_ramp(self,frequency=1,amplitude=1.0,onetimetrigger=False,offset=0,data=True):
        #corresponds to 2Vpp sine
        self.mode = "cosine"
        self.output_zero = True
        self.sm_reset=True
        ...
        if data:
            ...
            for i in range(len(self.d)):
                self.d[i] = np.long(np.round(-(2**13-1)*ramp((float(i)/2**14)*2*np.pi)))
            self.data = self.d
    ...
    def setup_halframp(self,frequency=1,amplitude=1.0,onetimetrigger=False,offset=0,data=True):
        #corresponds to 2Vpp sine
        self.mode = "cosine"
        self.output_zero = True
        self.sm_reset=True
    ...
    def dio_override(self,v):
        v=  v<<self.bit_offset | (self.read(0x40)&(~(0xFF<<self.bit_offset)))
        self.write(0x40,v)
...
class DspModule(BaseModule):
    ...
    def __init__(self,client,number=0):
        ...
        super(DspModule,self).__init__(client, addr_base=0x40300000+self.number*0x10000)
...
class FilterModule(DspModule):
    ...
    def inputfilter(self):
        ...
        for i in range(self._FILTERSTAGES):
            ...
            if filter_on:
                bandwidth = float(2**shift)/(2**alphabits)*125e6/2/np.pi
                if highpass:
                    bandwidth *= -1.0
            else:
                bandwidth = 0
    ...
    def inputfilter(self,v):
        ...
        for i in range(filterstages):
            ...
            if bandwidth == 0:
                ...
                shift = int(np.round(np.log2(np.abs(bandwidth)*(2**alphabits)*2*np.pi/125e6)))
                if shift < 0:
                    shift = 0
...
class IQ(FilterModule):
    ...
    def startphase_deg(self,v):
        self._start_phase = int(np.round((2**self._PHASEBITS)*float(v)/360.0))
    ...
    @property
    def amplitude(self):
        return float(self._g2)/2**self._SHIFTBITS/4.0 #same factor as in the setter

    @amplitude.setter
    def amplitude(self,v):
        """v is output signal amplitude in Volts at 50 Ohm output impedance. Cannot be larger than 0.5V"""
        v = float(v)*4.0 #experimentally found factor (could probably be calculated as well)
        maxvalue = 2**(self._GAINBITS-1)-1
        unityvalue = 2**self._SHIFTBITS
        g = int(np.round(float(v)*float(unityvalue)))
        if g > maxvalue:
    ...
    @property
    def frequency(self):
        return 125e6*float(self._shift_phase)/(2**self._PHASEBITS)*self._frequency_correction
    @frequency.setter
    def frequency(self,v):
        v=float(v)/self._frequency_correction
        self._shift_phase = int(np.round(float(v)/125e6*(2**self._PHASEBITS)))
    
    def set(self,frequency,bandwidth=[0],gain=1.0,phase=0,Q=None,inpuacbandwidth=50.,amplitude=0.0,inputport=1,outputport=1):
        self.on=False
        self.frequency = frequency
        if Q is None:
    ...
    def nadata(self):
        ...
        if not ((a>>31 == 0) and (b>>31 == 0) and (c>>31 == 0) and (d>>31 == 0)):
            print "Averaging not finished. Impossible to estimate value"
            return 0/0
        sum = np.complex128(self.to_pyint(a, bitlength=31)) + np.complex128(self.to_pyint(b,bitlength=31)*2**31) + 1j*np.complex128(self.to_pyint(c,bitlength=31)) + 1j*np.complex128(self.to_pyint(d,bitlength=31)*2**31)
    ...
    def na_trace(self,start=0,stop=100e3,points = 1001,rbw=100,avg=1.0,amplitude=1.0,input=1,output=1,acbandwidth=0.0,sleeptimes=0.5,logscale=False,raw=False):
        #sleeptimes*1/rbw is the stall time after changing frequency before averaging the quadratures
        unityfactor = 0.23094044589192711 #obtained by measuring transfer function with bnc cable
        if rbw < 1/34.0:
        ...
        y *= rescale
        if start==stop: #in zero-span mode, change x-axis to approximate time. Time is very rudely approximated here..
            x = np.linspace(0,self.na_time(points=points, rbw=rbw, avg=avg, sleeptimes=sleeptimes),points,endpoint=False)
        return x,y
    
    def na_trace_stabilized(self,start=0,stop=100e3,points = 1001,rbw=100,avg=1.0,stabilized=0.1,amplitude = 0.1,maxamplitude=1.0,input=1,output=1,acbandwidth=50.0,sleeptimes=0.5,logscale=False,raw=False):
        ...
        #sleeptimes*1/rbw is the stall time after changing frequency before averaging the quadratures
        unityfactor = 0.23094044589192711 #obtained by measuring transfer function with bnc cable. nominally 1/4
...
class IIR(DspModule):
    ...
    def iir_rawcoefficients(self):
        data = np.array([v for v in self.reads(0x28000+self.iir_channel*0x1000, 8*self.iir_loops)])
    ...
    def iir_coefficients(self):
        ...
        if l == 0:
            ...
            l=self._IIRSTAGES
        data = np.array([v for v in self.reads(0x28000+self.iir_channel*0x1000, 8*l)])
            ...
            for j in range(6):
                if j == 2:
                    ...
                    if j>3:
                        ...
                        k=j
                    coefficients[i,j] = self.to_double(data[i*8+2*k+1],data[i*8+2*k],bitlength=bitlength,shift=shift)
            ...
            for j in range(6):
                if j == 2:
                    ...
                    if j>3:
                        ...
                        k=j
                    hi, lo = self.from_double(v[i,j],bitlength=bitlength,shift=shift)
                    data[i*8+k*2+1] = hi
