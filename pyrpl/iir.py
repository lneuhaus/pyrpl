...
def rpk2psos(r,p,k,tol=0,verbose=True):
    ...
    if len(linr)==0:
        ...
        #now all multiple occurences of poles should have disappeared from linp
...
def get_coeff_old(sys,dt=8e-9,totalbits=64,shiftbits=32,tol=0,finiteprecision=False):
    zc,pc,kc = sys
    if zc == [] and pc == []:
    ...
    if finiteprecision:
        fcoeff = finiteprecision(coeff,totalbits=totalbits,shiftbits=shiftbits)
        print "Rounded coefficients: ",fcoeff
        return fcoeff
    else:
        return coeff

def get_coeff(sys,dt=8e-9,totalbits=64,shiftbits=32,tol=0,finiteprecision=False):
    """
...
def example():
    plt.close()
    pc=2.0*np.pi*np.array([(-0.5-13428.513784154658j), (-0.5+13428.513784154658j),(-10000+0j),(-11e4+0j)],dtype=np.complex128)
