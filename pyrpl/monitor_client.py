...
class MonitorClient(object):
    ...
    def close(self):
        try:
            self.socket.send('c'+chr(0)+chr(0)+chr(0)+chr(0)+chr(0)+chr(0)+chr(0))
            self.socket.close()
        except socket.error:
            return
    ...
    def _reads(self, addr, length):
        ...
        header = 'r'+chr(0) +chr(length&0xFF)+chr((length>>8)&0xFF) \
                 +chr(addr&0xFF)+chr((addr>>8)&0xFF)+chr((addr>>16)&0xFF)+chr((addr>>24)&0xFF)
    ...
    def _writes(self, addr, values):
        ...
        length = len(values)
        header = 'w'+chr(0) +chr(length&0xFF)+chr((length>>8)&0xFF) \
                 +chr(addr&0xFF)+chr((addr>>8)&0xFF)+chr((addr>>16)&0xFF)+chr((addr>>24)&0xFF)
        self.socket.send(header+str(np.getbuffer(np.array(values,dtype=np.uint32)))) #send header+body
    ...
    def restart(self):
        self.close()
        self._restartserver()
        self.__init__(address=self._address,port=self._port,restartserver=self._restartserver)
