...
#unspecified parameters should rather be left unchanged instead of being set to 0 or 1
...
CONSTANTS_DEFAULT = dict(
        ...
        relative_reflection_off = 0.01,     #detect that laser is off and return immediately for relock of prior cavities
...
class Pyrpl(Lockbox):
    def __init__(self, constants=None, cavity='FPF2', reloadfpga=True):
        ...
        self.rp = RedPitaya(hostname=self.constants["hostname"],autostart=True,reloadfpga=reloadfpga,filename=self.constants["fpgafile"],dirname=self.constants["fpgadir"],frequency_correction=self.constants["frequency_correction"])
        ...
        self.sof = self._get_pid(input=self.constants["reflection_input"],output=self.constants["lock_output"])
        if "lock_aux_output" in self.constants and self.constants["lock_aux_output"] != 0:
            self.sof_aux = self._get_pid(input=self.constants["reflection_input"],output=self.constants["lock_aux_output"])
        else:
            self.sof_aux = None
    ...
    def _get_min_step(self,output="coarse"):
        if output == "coarse":
            if (self.constants["coarse_output"]==1 or self.constants["coarse_output"]==2):
                resolution = 14
    ...
    def _get_signal(self,signal_number,type='input',avg=0,do_rms=False,minmax=False,trace=False,trace_frequency=10e3): 
        #avg = 0: take a whole scope trace of coherent data
        #avg >0: take avg individual samples, not necessarily uniformly spaced in time
        if avg == 1:
        ...
        if type == 'output': #swap signal 1 and 2 as scope channel 2 records output 1 and the other way around
            if signal_number == 1:
                ...
                signal_number = 1
        self.scope_reset()
        beforestate = [self.s.dac2_on_ch1,self.s.quadrature_on_ch1,self.s.dac1_on_ch2,self.s.quadrature_on_ch2]
        if signal_number == 1:
            if type == 'input':
        ...
        if avg == 0:
            self.s.arm(frequency=self.constants["mean_measurement_frequency"],trigger_source=1)
            sleep(1.0/self.s.frequency)
            if signal_number == 1:
    ...
    def get_mean(self,signal="reflection",avg=0,rms=False,minmax=False):
        if signal == 1 or signal == 2:
            return self._get_signal(signal_number = signal,type="input",avg=avg,do_rms=rms,minmax=minmax)
        elif signal == 3:
            return self._get_signal(signal_number = 1, type="quadrature",avg=avg,do_rms=rms,minmax=minmax)
        elif signal == "reflection":
            return self._get_signal(signal_number = self.constants["reflection_input"],type="input",avg=avg,do_rms=rms,minmax=minmax)
        elif signal == "pdh" or signal=="lock":
            channel = self.constants[signal+"_input"]
            if channel == 3:
                return self._get_signal(signal_number = 1, type="quadrature",avg=avg,do_rms=rms,minmax=minmax)
            else:
                return self._get_signal(signal_number = channel, type="input",avg=avg,do_rms=rms,minmax=minmax)
        elif signal == "output":
            channel = self.constants["lock_output"]
            return self._get_signal(signal_number = channel,type="output",avg=avg,do_rms=rms,minmax=minmax)
        else:
            return None
    ...
    def reflection_mW(self):
        return (self.get_mean("reflection",avg=1)-self.constants["dark_reflection"])*self.constants["calibration_slope"]

    def _relative_reflection(self,refl):
        return float(refl-self.constants["dark_reflection"])\
               /float(self.constants["offres_reflection"]-self.constants["dark_reflection"])
    @property
    ...
    @property
    def laser_off(self):
        if (self.relative_reflection < self.constants["relative_reflection_off"]):
    ...
    def errorsignals(self,frequency=None):
        ...
        inputnames = list()
        dacmode = False
        if self.constants["reflection_input"] == 1 or self.constants["reflection_input"] == 2:
            input.append(self.constants["reflection_input"])
            inputnames.append("error reflection")
        if self.constants["pdh_input"] == 1 or self.constants["pdh_input"] == 2:
        ...
        if "last_sbreflection_curve" in self.constants: 
            CurveDB.objects.get(pk=self.constants["last_sbreflection_curve"]).add_child(c2)
        elif "last_reflection_curve" in self.constants: 
            CurveDB.objects.get(pk=self.constants["last_reflection_curve"]).add_child(c2)
        if "last_pdh_curve" in self.constants:
            CurveDB.objects.get(pk=self.constants["last_pdh_curve"]).add_child(c3)
        if "last_calibration_reflection" in self.constants:
            CurveDB.objects.get(pk=self.constants["last_calibration_reflection"]).add_child(c2)
        if "last_calibration_pdh" in self.constants:
            CurveDB.objects.get(pk=self.constants["last_calibration_pdh"]).add_child(c3)        
    ...
    def _coarse_setter(self,v):
        setV = self.constants["coarse_min_volt"]*(1.0-v) + self.constants["coarse_max_volt"]*v
    ...
    def darknoise(self,avg=0):
        ...
        constants = dict( last_dark_reflection = self.constants["dark_reflection"],
    ...
    def calibrate_power(self,power):
        ...
        """
        r = self.get_mean(signal="reflection") - self.constants["dark_reflection"]
        constants = dict(calibration_power = power, calibration_reflection = r, calibration_slope=power/r)
        self.constants.update(constants)
        self._save_constants(constants)
    ...
    def power_offresonant(self):
        r = self.constants["offres_reflection"] - self.constants["dark_reflection"]
        return r*self.constants["calibration_slope"]
    ...
    def _outlen(self):
        ...
        return np.round(float(self.f.lastpoint+1)/float(self.s.data_decimation))%self.s.data_length
    @property
    def _outi(self):
        points = self._outlen
        d = np.cos(np.linspace(0.0,2*np.pi,points,endpoint=False))*float(-(2**13-1))
        return d
    @property
    def _outq(self):
        decimation = self.s.data_decimation
        lastpoint = self.f.lastpoint
        points = np.long(np.round(float(self.f.lastpoint+1)/float(self.s.data_decimation)))
        d = np.sin(np.linspace(0.0,2*np.pi,points,endpoint=False))*float(-(2**13-1))
        return d
    ...
    def _outsin(self,amplitude=None):
        ...
        points = self._outlen
        d = amplitude * np.cos(np.linspace(0.0,2*np.pi,points,endpoint=False))*float(-(2**13-1))
        return d
    ...
    def _outramp(self,amplitude=None):
        if amplitude is None:
            amplitude = self.constants["sweep_amplitude"]
        points = self._outlen
        d = amplitude * (np.abs((np.linspace(-2.0,2.0,points,endpoint=False)))-1.0)*float(-(2**13-1)) 
        return d
    ...
    def _sweep_setup(self,amplitude=None,frequency=None,dacmode=0):
        ...
        self.f.setup_cosine(frequency=frequency,amplitude=amplitude,onetimetrigger=True)
        self.s.setup(frequency=frequency,trigger_source=8,dacmode=dacmode)
    
    def sweep_coarse(self,amplitude=None,frequency=None,waveform="ramp",offset=None,onetimetrigger=False):
        ...
        if self.constants["coarse_output"] == 1:
            ...
            return
        if waveform == "ramp":
            f.setup_ramp(frequency=frequency,amplitude=amplitude,onetimetrigger=onetimetrigger,offset=f.offset)
        else:
            f.setup_cosine(frequency=frequency,amplitude=amplitude,onetimetrigger=onetimetrigger,offset=f.offset)
    ...
    def scope_reset(self):
        self.s.setup(frequency=self.constants["mean_measurement_frequency"],trigger_source=1,dacmode=False)
    ...
    def scope_trace(self, input=[1,2], frequency=1000.0, trigger_source = 1, name_prefix="",dacmode=False):
        """
        ...
        self.s.setup(frequency=frequency,trigger_source=trigger_source,dacmode=dacmode)
        self.s.arm(trigger_source = trigger_source, trigger_delay = int(self.s.data_length/2))
        if trigger_source == 1:
        ...
        for i in range(len(input)):
            ...
            c.params.update(dict(input=input[i],frequency=frequency,trigger_source=trigger_source))
            c.save()
            curves.append(c)
        return tuple(curves)

    def sweep_acquire(self, input=1, output = 1, frequency=None, amplitude=None, waveform = None, data_shorten=True):
        """
        acquires the data of an input during one sweep
        """
        if output == 2:
        ...
        if amplitude is None:
            amplitude = self.constants["sweep_amplitude"]
        #amplitude *= 8191.0 #convert from volts to counts - now this is implemented in monitor.py already
        if frequency is None:
            frequency = self.constants["sweep_frequency"]
        if waveform is None:
            waveform = self.constants["sweep_waveform"]
        if waveform == "ramp":
            f.setup_ramp(frequency=frequency,amplitude=amplitude,onetimetrigger=True)
        elif waveform == "halframp":
            f.setup_halframp(frequency=frequency,amplitude=amplitude,onetimetrigger=True)
        else: #"sine"
            f.setup_cosine(frequency=frequency,amplitude=amplitude,onetimetrigger=True)
    ...
    def quadratures_acquire(self,frequency=None,times=False):
        ...
        sleep(1.0/self.s.frequency)
        q1 = np.roll(self.rp.getbuffer(self.s.rawdata_ch1),-(self.s.write_pointer_current+1))
        q2 = np.roll(self.rp.getbuffer(self.s.rawdata_ch2),-(self.s.write_pointer_current+1))
        if times:
    ...
    def spectrum_trace(self,name="rp_spectrum",input=1,frequency=1e3,kaiserbeta=15.0,avg=1):
        sumspectrum = None
        for i in range(avg):
            self.s.setup(frequency=frequency,trigger_source=0)
            self.s.trigger_source=1
            sleep(1.0/self.s.frequency)
            if input==1:
                data = np.roll(self.rp.getbuffer(self.s.rawdata_ch1),-(self.s.write_pointer_current+1))
            else:
                data = np.roll(self.rp.getbuffer(self.s.rawdata_ch2),-(self.s.write_pointer_current+1))
    ...
    def na_trace(self,input=1,output=1,start=0,stop=100e3,points=1001,rbw=100,avg=1.0,amplitude=0.1,autosave=True,name="rpna_curve",sleeptimes=0.5,stabilized=None,maxamplitude=0.2):
        ...
        if stabilized is None:
            ...
            x,y,z=self.iq.na_trace_stabilized(input=input,output=output,start=start,stop=stop,
                             points=points,rbw=rbw,avg=avg,amplitude=amplitude,sleeptimes=sleeptimes,maxamplitude=maxamplitude,stabilized=stabilized)
        ...
        if autosave:
            ...
            c.name = name
            d = dict(start=start,stop=stop,input=input,
                     output=output,points=points,rbw=rbw,avg=avg,amplitude=amplitude,saturation_db=saturation_db,
                     hostname = self.constants["hostname"],stabilized=stabilized,maxamplitude=maxamplitude,
                     type = "RPNAcurve")
            c.params.update(d)
            c.save()
    ...
    def optimize_gain(self,output=None,deviation=0.3,p_or_i="p",points=31,avg=0):
            ...
            for sign in [1.0,-1.0]:
                ...
                if self.islocked:
                    if input==self.constants["pdh_input"]:
                        ...
                        prms.append(pms)
                    r,rms=self.get_mean(signal="reflection", avg=avg, rms=True)
                    rrms.append(rms)
                    gains.append(gain)
        if p_or_i=="i":
            pid.integral = center
        ...
        norm_r = self.constants["offres_reflection"]-self.constants["dark_reflection"]
    ...
    def pdh_max(self):
        ...
        #return self.constants["pdh_max"]*self.constants["offres_reflection"]/self.constants["offres_reflection_optimum"]
    ...
    def unlock(self,jump=None):
        ...
        for pid in [self.rp.pid11,self.rp.pid12,self.rp.pid21,self.rp.pid22]:
            pid.reset=True
            pid.integral=0
            pid.proportional=0
            pid.derivative=0
    ...
    def _params(self):
        r,rrms,rmin,rmax = self.get_mean(signal="reflection",rms=True,minmax=True)
        p,prms,pmin,pmax = self.get_mean(signal="pdh",rms=True,minmax=True)
        dic = dict()
        dic.update({
...
#IIR section
    def setup_iir(self,sys,input=1,output=1,acbandwidth=None,loops=None,plot=True,save=False,turn_on=True,tol=1e-6):
        """Setup an IIR filter
        -----------------------------
        sys = (z,p,k)
        ...
        #nasty bugfix here: needs cleanup of fpga code. avoids disconnecting iq0 from its real input
        ...
        if plot or save:
            if type(plot) == int:
                ...
                plt.figure()
            tfs = iir.psos2plot(c,sys,n=2**16,maxf=5e6,dt=dt,name="discrete system (dt="+str(int(dt*1e9))+"ns)")
            tfs += iir.psos2plot(f,None,n=2**16,maxf=5e6,dt=dt,name="implemented system")
            plt.legend()
        if save:
            if not plot:
                plt.close()
    ...
    def init_iir(self,plot=False,save=False,tol=1e-3,input=None,output=None):
        if input is None:
            if self.stage >= PDHSTAGE:
                input = self.constants["pdh_input"]
            else:
        ...
        ret = self.setup_iir((z,p,k),acbandwidth=acbandwidth,input=input,output=0,loops=loops,turn_on=True,plot=plot,tol=tol,save=save)
        if output is None:
            if hasattr(self,"pidpdh"):
                if self.stage >= PDHSTAGE:
    ...
    def set_optimal_gains(self,save=True):
        ...
        #              sof_aux_i = self.sof_aux.integral,sof_aux_p = self.sof_aux.proportional)
        if self.sof.integral != 0 or self.sof.proportional != 0:
            print "Estimating gains from sof lock..."
            if type(self.slope)==float or type(self.slope)==double:
                slope = self.slope
            else:
                slope = self.slope(signal = "reflection",value=self.sof.setpoint)
            d = dict(lock_i_times_slope = float(self.sof.integral)*slope*self.constants["isr_correction"],
                     lock_p_times_slope = float(self.sof.proportional)*slope,
                     slope_optimum = slope)
            ...
            if not self.sof_aux is None:
                d.update(dict(  lock_aux_i_times_slope = float(self.sof_aux.integral)*slope*self.constants["isr_correction"],
                                lock_aux_p_times_slope = float(self.sof_aux.proportional)*slope))
            ...
            d = dict(lock_i_times_slope = float(self.pidpdh.integral)*slope*self.constants["isr_correction"],
                     lock_p_times_slope = float(self.pidpdh.proportional)*slope,
                     slope_optimum = slope)
            ...
            if not self.sof_aux is None:
                d.update(dict(  lock_aux_i_times_slope = float(self.pidpdh_aux.integral)*slope*self.constants["isr_correction"],
                                lock_aux_p_times_slope = float(self.pidpdh_aux.proportional)*slope))
...
class Pyrpl_FP(Pyrpl):
    def find_coarse(self,start=0.0,stop=1.0):
        ...
        find_coarse_threshold = int(self.constants["dark_reflection"]+self.constants["find_coarse_upper_threshold"]*\
        ...
        for act_iteration in range(self.constants["find_coarse_max_iterations"]):
            #upwards search
            self.s.arm(trigger_source = trigger_source)
            print "sweeping upwards..."
            steps,delay = self._get_stepdelay("coarse",stop-start,searchtime)
    ...
    def from_detuning(self,detuning_in_bandwidths,signal="reflection"):
        if signal == "reflection":
            ...
            if self.constants["pdh_offsetcorrection"]:
                ...
                offset_pdh = 0
            return offset_pdh + pdhmax*2*detuning_in_bandwidths/(1.0+detuning_in_bandwidths**2)
    ...
    def to_detuning(self,value=None,signal="reflection"):
        if signal == "reflection":
            ...
            if value == 0:
                ...
                detuning_in_bandwidths = (1.0-np.sqrt(np.max([0,1.0-(value/pdhmax)**2])))/(value/pdhmax)
            return detuning_in_bandwidths
    
    def get_detuning(self):
        if self.sof.integral != 0:
            actdetuning = self.to_detuning(value=self.sof.setpoint, signal="reflection")
        elif self.pidpdh.integral != 0:
            actdetuning = self.to_detuning(value=self.pidpdh.setpoint, signal="pdh")
        else:
    ...
    def slope(self,value=None,signal="reflection"):
        if signal == "reflection":
            ...
            detuning_in_bandwidths = self.to_detuning(value=value,signal = "reflection")
            slope = 2*scale*detuning_in_bandwidths/(1.0+detuning_in_bandwidths**2)**2
            #if maxcorrection: #correct for gain enhancement by the resonance at large detunings
            if self.constants["sof_maxcorrection"]:
                d2 = detuning_in_bandwidths**2
                excessgain = (1.+d2)*np.sqrt(1.0+5*d2+4*d2**2)/(1.0+4*d2)
                slope *= excessgain
        elif signal == "pdh":
            if value is None:
                value = self.pdh
            """pdh signal is - when the detuning is betzeen -1 and +1 cavity bandwidths - 
               ...
               analytical one:"""
            pdhmax = self.pdh_max
            detuning_in_bandwidths = self.to_detuning(value=value,signal="pdh")
            slope = 2*pdhmax*(1.0-detuning_in_bandwidths**2)/(1.0+detuning_in_bandwidths**2)**2
        if np.abs(slope) < np.abs(self.constants["slope_lower_limit"]*self.constants["slope_optimum"]):
            if slope < 0:
                slope = np.abs(self.constants["slope_lower_limit"]*self.constants["slope_optimum"])*(-1.0)
            else:
                slope = np.abs(self.constants["slope_lower_limit"]*self.constants["slope_optimum"])
        return slope
        
    def lock_opt(self,detuning=None,sof=None,pdh=None,time=0):
        aux = ("lock_aux_output" in self.constants) and (self.constants["lock_aux_output"] != 0)
        if pdh != 0 and not self.pdhon: #turn on the pdh if necessary
            self.setup_pdh()
        if detuning is None:
            detuning = self.get_detuning()
        if time == -1:
            sof_sp = np.round(self.from_detuning(detuning,signal="reflection"))
            ...
            sofintegral = self.constants["lock_i_times_slope"]/self.constants["isr_correction"]/sof_slope*sof
            if np.abs(sofintegral) <= 0.5:
                self.sof.integral = np.sign(sofintegral)
            else:
                self.sof.integral = np.round(sofintegral)
            self.sof.proportional = np.round(self.constants["lock_p_times_slope"]/sof_slope*sof)
            pdhintegral = self.constants["lock_i_times_slope"]/self.constants["isr_correction"]/pdh_slope*pdh
            if np.abs(pdhintegral) <= 0.5:
                self.pidpdh.integral = np.sign(pdhintegral)
            else:
                self.pidpdh.integral = np.round(pdhintegral)            
            self.pidpdh.proportional = np.round(self.constants["lock_p_times_slope"]/pdh_slope*pdh)
            if aux:
                self.sof_aux.setpoint = sof_sp
                self.pidpdh_aux.setpoint = pdh_sp
                self.sof_aux.integral = np.round(self.constants["lock_aux_i_times_slope"]/self.constants["isr_correction"]/sof_slope*sof)
                self.sof_aux.proportional = np.round(self.constants["lock_aux_p_times_slope"]/sof_slope*sof)
                self.pidpdh_aux.integral = np.round(self.constants["lock_aux_i_times_slope"]/self.constants["isr_correction"]/pdh_slope*pdh)
                self.pidpdh_aux.proportional = np.round(self.constants["lock_aux_p_times_slope"]/pdh_slope*pdh)
            return
        else:
            if aux:
                instructions = 12
            ...
            sof_slope = self.slope(sof_sp,signal="reflection")
            pdh_slope = self.slope(pdh_sp,signal="pdh")
            actsof = self.sof.integral*sof_slope/self.constants["lock_i_times_slope"]*self.constants["isr_correction"]
            if actdetuning < self.to_detuning(value=self.pdh_max,signal="pdh"):
                actpdh = self.pidpdh.integral*pdh_slope/self.constants["lock_i_times_slope"]*self.constants["isr_correction"]
            else:
                actpdh = 0
            if sof is None:
                sof = actsof
            ...
            if pdh==0 and sof==0:
                sof = 1.0
            steps,delay = self._get_stepdelay(output = "pid", range = 8192.0, duration = time, instructions=instructions)
            if self.constants["verbosity"]:
                    print "SOF-SP,I,P,PDH-SP,I,P:"
            for f in linspace(0.0,1.0,steps,endpoint = True):
                self.lock_opt(detuning = f*detuning+(1.0-f)*actdetuning,
                              sof=f*sof+(1.0-f)*actsof,pdh=f*pdh+(1.0-f)*actpdh,time=-1)
                if self.constants["verbosity"]:
    ...
    def lock(self,detuning=None,laststage=None):
        ...
        r0,rms = self.get_mean("reflection", avg=0, rms=True) # measure reflection
        ...
        self.coarse += self.constants["drift_jump"]#*self.get_min_step("coarse") #go on high_voltage side of the resonance
        ...
        self.lock_opt(detuning=self.constants["drift_detuning"], time=0, sof=self.constants["drift_factor"])
        while time()<timeout:
            if self.reflection < self.from_detuning(self.constants["drift_detuning"]+rms, signal="reflection"):
                break
            else:
        ...
        if self.constants["pdh_offsetcorrection"]:
            ...
            offset_pdh = 0
        if self.islocked:
            self.constants["pdh_max_last"] = p0-offset_pdh#max([np.abs(pmin-offset_pdh),np.abs(pmax-offset_pdh)])
        self.lock_opt(detuning=detuning, sof=0.0,pdh=1.0,time=self.constants["pdh_time"]*0.75)
    ...
    def relock(self,detuning=None,sof=0,pdh=0):
        ...
        if sof==0 and pdh==0:
            if laststage >= PDHSTAGE:
                ...
                sof,pdh = 1,0
        if self.islocked:
            self.lock_opt(detuning=detuning,sof=sof,pdh=pdh,time=self.constants["relocktime"])
        for i in range(5):
    ...
    def _raw_lock(self,drift_sp=None,drift_i=None,drift_p=None,drift_aux_i=None,drift_aux_p=None,
                  sof_sp=None,sof_i=None,sof_p=None,sof_aux_i=None,sof_aux_p=None,
                  pdh_sp=None,pdh_i=None,pdh_p=None,pdh_aux_i=None,pdh_aux_p=None,
                  laststage=None):
        ...
        drift_sp = (r0-rd) * drift_sp + rd #setpoint is upper threshold for considering the cavity locked
        ...
        steps,delay = self._get_stepdelay(output = "pid", range = sof_sp-drift_sp, duration = sof_time, instructions=3+extrainstructions)
        ...
        for f in linspace(0.0,1.0,steps,endpoint = True):
            ...
            if not self.sof_aux is None:
                self.sof_aux.setpoint = self.sof.setpoint
                self.sof_aux.integral = np.round(f*sof_aux_i+(1.0-f)*drift_aux_i)
                self.sof_aux.proportional = np.round(f*sof_aux_p+(1.0-f)*drift_aux_p)
            if self.constants["verbosity"]:
        ...
        p0,prms = self.get_mean("pdh",avg=0,rms=True) #estimate pdh setpoint equivalent
        if pdh_sp is None:
        ...
        steps,delay = self._get_stepdelay(output = "pid", range = max([abs(p0-pdh_sp),abs(pdh_i),abs(pdh_p),abs(sof_i),abs(sof_p)]),
                                                         duration = pdh_time, instructions=6+extrainstructions)
    ...
    def _setup_pdh(self):
        if (self.constants["pdh_input"] == 3): #means internal demodulation
            self.iq.iq_set_advanced(channel = 0,
            ...
            self.iq.iq_i0_factor = np.round(self.constants["pdh_factor"] * self.constants["pdh_referencereflection"]/
                                    (self.constants["offres_reflection"]-self.constants["dark_reflection"]))
            if self.constants["reflection_input"] == 1:
                free_input = 2   #need to find the free pid controllers (the ones not used for sof locking)
            else:
                free_input = 1
            self.pidpdh = self._get_pid(input = free_input, output = self.constants["lock_output"])
            self.pidpdh.inputiq = True
            if self.constants["lock_aux_output"] != 0:
                self.pidpdh_aux = self._get_pid(input = free_input, output = self.constants["lock_aux_output"])
            ...
            self.pidpdh = self._get_pid(input = self.constants["pdh_input"], output = self.constants["lock_output"])
            self.pidpdh.inputiq = False
            if self.constants["lock_aux_output"] != 0:
                self.pidpdh_aux = self._get_pid(input = self.constants["pdh_input"], output = self.constants["lock_aux_output"])
    ...
    def optimize_pdh(self,start=0,stop=360,steps=37,sweepamplitude=0.3,sweepfrequency=10):
        fgenphase=None
        c1=self.find_coarse()
        c2=self.find_coarse()
        self.coarse = (c1+c2)/2.0   
        self.sweep_coarse(amplitude=sweepamplitude,frequency=sweepfrequency,offset=self.coarse)
        self.setup_pdh()
        self.iq.iq_channel = 0
        ...
        if self.constants["reflection_input"] == 1:
            ...
            trigger_source = 2
            self.s.threshold_ch1 = threshold
        #   print "Remember to subtract 90 degrees from the final phase before putting it in the dictionary"
        ...
        self.s.setup(frequency = self.f.frequency*self.constants["cavity_finesse"]/30.0,trigger_source=self.s.trigger_source,dacmode=True)
        ...
        for i,p in enumerate(phases):
            ...
            while True:
                if self.s.trigger_source == 0:
                    scopetrigphase = self.f.scopetrigger_phase #make sure all curves are taken on ascending slope of function generator
                    ...
                    if (fgenphase <= scopetrigphase) and (scopetrigphase < fgenphase+180.0):
                        break
                    else:
                        self.s.arm(trigger_source=trigger_source,trigger_delay=0.5)
                        continue
    ...
    def align_acoustic(self,normalfrequency=20000.0,sosfrequency=3000.,verbose=True):
